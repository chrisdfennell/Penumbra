...
### Changed
- **Weather row** moved down for better balance under the date, and the daily high/low
  are each now capped with a caret — an up-chevron over the high, a down-chevron over the
  low — replacing the old `hi/lo` slash.

### Added
- Up/down **caret icons** (SVG, light and dark variants) drawn above the weather
