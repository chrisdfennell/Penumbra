param(
    # Default to the 454x454 AMOLED product (fenix 8 47mm). Pass -Device <id> to
    # build/preview another variant, or -Export to package the store .iq.
...
if ($Export) {
    Write-Host "Package Succeeded! Output: bin\Penumbra.iq" -ForegroundColor Green
} else {
    Write-Host "Build Succeeded! Output: bin\Penumbra.prg" -ForegroundColor Green
}
